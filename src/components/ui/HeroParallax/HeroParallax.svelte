...
<div
  bind:this={ref}
  class="h-[300vh] relative flex flex-col self-auto overflow-hidden py-4 antialiased [perspective:1000px] [transform-style:preserve-3d]"
>
  <div
    class="relative left-0 top-20 mx-auto w-full max-w-7xl px-4 py-8 md:py-4"
  >
    <h2 class="text-2xl font-bold dark:text-white md:text-7xl">About Us</h2>
    <p class="mt-8 max-w-2xl text-base dark:text-neutral-200 md:text-xl">
      We're VPCI's one and only robotics team.
    </p>
  </div>
  <Motion
    let:motion
  ...
  >
    <div use:motion>
      <Motion let:motion>
        <div
          use:motion
          class="mb-20 flex flex-row-reverse space-x-20 space-x-reverse"
        >
      ...
      <Motion let:motion>
        <div
          use:motion
          class="flex flex-row-reverse space-x-20 space-x-reverse"
        >
